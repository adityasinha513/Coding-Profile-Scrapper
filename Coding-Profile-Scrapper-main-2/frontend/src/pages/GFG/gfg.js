  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const gfgProfile = response.data.find(p => p.Platform === 'GFG');
        if (gfgProfile) {
          setProfile(gfgProfile);
        } else {
          setError('GFG profile not found');
        }
        setLoading(false);
      } catch (err) {
        console.error('Error fetching GFG profile:', err);
        setError(err.response?.data?.message || 'Failed to fetch GFG profile');
        setLoading(false);
      }
    };
