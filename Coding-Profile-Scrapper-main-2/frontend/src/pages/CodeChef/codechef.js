  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const codechefProfile = response.data.find(p => p.Platform === 'CodeChef');
        if (codechefProfile) {
          setProfile(codechefProfile);
        } else {
          setError('CodeChef profile not found');
        }
        setLoading(false);
      } catch (err) {
        console.error('Error fetching CodeChef profile:', err);
        setError(err.response?.data?.message || 'Failed to fetch CodeChef profile');
        setLoading(false);
      }
    };
